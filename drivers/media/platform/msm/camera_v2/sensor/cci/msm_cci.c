	...
	enum cci_i2c_queue_t queue)
{
	...
	if ((read_val + len + 1) > cci_dev->
		...
		spin_lock_irqsave(&cci_dev->cci_master_info[master].lock_q[queue], flags);
		...
		spin_unlock_irqrestore(&cci_dev->cci_master_info[master].lock_q[queue], flags);
...
static irqreturn_t msm_cci_irq(int irq_num, void *data)
{
	...
	if (irq & CCI_IRQ_STATUS_0_I2C_M0_Q0_REPORT_BMSK) {
		...
		spin_lock_irqsave(&cci_dev->cci_master_info[MASTER_0].lock_q[QUEUE_0], flags);
		...
		spin_unlock_irqrestore(&cci_dev->cci_master_info[MASTER_0].lock_q[QUEUE_0], flags);
	}
	if (irq & CCI_IRQ_STATUS_0_I2C_M0_Q1_REPORT_BMSK) {
		struct msm_camera_cci_master_info *cci_master_info;
		cci_master_info = &cci_dev->cci_master_info[MASTER_0];
		spin_lock_irqsave(&cci_dev->cci_master_info[MASTER_0].lock_q[QUEUE_1], flags);
		...
		spin_unlock_irqrestore(&cci_dev->cci_master_info[MASTER_0].lock_q[QUEUE_1], flags);
	}
	...
	if (irq & CCI_IRQ_STATUS_0_I2C_M1_Q0_REPORT_BMSK) {
		...
		spin_lock_irqsave(&cci_dev->cci_master_info[MASTER_1].lock_q[QUEUE_0], flags);
		atomic_set(&cci_master_info->q_free[QUEUE_0], 0);
		cci_master_info->status = 0;
		if (atomic_read(&cci_master_info->done_pending[QUEUE_0]) == 1) {
			complete(&cci_master_info->report_q[QUEUE_0]);
		...
		spin_unlock_irqrestore(&cci_dev->cci_master_info[MASTER_1].lock_q[QUEUE_0], flags);
	}
	if (irq & CCI_IRQ_STATUS_0_I2C_M1_Q1_REPORT_BMSK) {
		struct msm_camera_cci_master_info *cci_master_info;
		cci_master_info = &cci_dev->cci_master_info[MASTER_1];
		spin_lock_irqsave(&cci_dev->cci_master_info[MASTER_1].lock_q[QUEUE_1], flags);
		atomic_set(&cci_master_info->q_free[QUEUE_1], 0);
		cci_master_info->status = 0;
		if (atomic_read(&cci_master_info->done_pending[QUEUE_1]) == 1) {
			complete(&cci_master_info->report_q[QUEUE_1]);
		...
		spin_unlock_irqrestore(&cci_dev->cci_master_info[MASTER_1].lock_q[QUEUE_1], flags);
	}
